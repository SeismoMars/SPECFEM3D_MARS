...
!--------------------------------------------------------------------------------------------------
  ...
  ! Youyi MARS 09/29/2015
  integer, parameter :: CRUSTMAP_RESOLUTION = 6 !means 1/6 degrees
  !integer, parameter :: CRUSTMAP_RESOLUTION = 1 !means 1 degrees
...
!
  ...
  !=== Youyi MARS 09/29/2015
  !write(eucrust,'(a,a1,i1,a)') 'DATA/crustmap/eucrust', var_letter, ind, '.cmap'
  write(eucrust,'(a,a1,i1,a)') 'DATA/crustmap/marscrust', var_letter, ind, '.cmap'
...
! else if (x > x4 .and. INCLUDE_SEDIMENTS_IN_CRUST .and. h_sed > MINIMUM_SEDIMENT_THICKNESS) then
  ...
  if (found_crust) then
    !== test Youyi Ruan 10/16/2016
    !if(lat > 30. .and. lat < 31. .and. lon > 120. .and. lon < 121. ) then
    !  write(*,'(a,2F9.2,3F10.2)') "DEBUG: 30<lat<31 120<lon<121:",lat,lon,(1.0 - x)*R_EARTH/1000.,vs,h_uc+thicks(4)+thicks(5)
    !endif
    !=== 
    ! non-dimensionalize
  ...
  !=== Youyi Ruan Mars 10/16/2016
  ! comment out this part for test. doesn't seem make sense to set
  ! the values
  !===
  !else
  !  scaleval = dsqrt(PI*GRAV*RHOAV)
...
! crustal vp and vs in km/s, layer thickness in km
  ...
  integer, parameter :: NTHETA = 8 ! 4      
  integer, parameter :: NPHI = 60  ! 20
  !-------------------------------
...
! -90<lat<90 -180<lon<180
  ...
  !=== Youyi Ruan 10/16/2016
  !=== comment out following to remove the regional only feature 
