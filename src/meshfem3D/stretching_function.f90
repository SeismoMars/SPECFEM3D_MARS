...
!
  ...
  if (ner == 3) then
    ...
    stretch_tab(1,2) = 3370000.d0  ! 3353 second layer top  doesn't work well, give negative Jacobian
    stretch_tab(1,3) = 3307000.d0  ! 3317 third layer top

    stretch_tab(2,1) = 3343000.d0  ! 3353 bottom first layer
    stretch_tab(2,2) = 3307000.d0  ! 3317 bottom second layer
    stretch_tab(2,3) = r_bottom    ! bottom third layer
  endif 

  !=== Youyi Ruan 11/17/2016 MARS
  ! To capture the large variation of Mars crustal thickness, we introduce 
  ! 5 element layers in crust defined below and then stretching these layer 
  ! boundaries in moho_stretching.f90 
  ! Note the values of boundary radius need to be the same in moho_stretching 
  ! to make sure the moho will be strictly honored. 
  !=== 
  if (ner == 5) then
    stretch_tab(1,1) = r_top
    stretch_tab(1,2) = 3375000.d0  ! 2nd layer top
    stretch_tab(1,3) = 3355000.d0  ! 3rd layer top
    stretch_tab(1,4) = 3335000.d0  ! 4th layer top
    stretch_tab(1,5) = 3310000.d0  ! 5th layer top
    ...
    if (r_bottom >= stretch_tab(1,5)) then
      print *, "ERROR: wrong layer in regional stretching table!"
    endif
  endif
