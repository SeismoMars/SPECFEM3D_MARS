...
!---
  ...
  if (MODEL_ROOT == '1D_isotropic_prem') then
    ...
    TRANSVERSE_ISOTROPY = .true.
          ...
          MODEL_ROOT == '1D_sea99' .or. MODEL_ROOT == '1D_Sohl') then
    HONOR_1D_SPHERICAL_MOHO = .true.
    ...
    else if (MODEL_ROOT == '1D_Sohl') then
  ...
  ! 3-D models
    ...
    ONE_CRUST = .true.
    TRANSVERSE_ISOTROPY = .true.
  ! Added by Youyi Ruan for Mars 09/26/2015 
  else if (MODEL_ROOT == '1D_Sohl_3D_crust') then
    CASE_3D = .true.
    CRUSTAL = .true.
    ONE_CRUST = .true.
    REFERENCE_1D_MODEL = REFERENCE_MODEL_SOHL
  ...
  if ((REFERENCE_1D_MODEL == REFERENCE_MODEL_IASP91 .or. &
      ...
      REFERENCE_1D_MODEL == REFERENCE_MODEL_SOHL .or. &
      REFERENCE_1D_MODEL == REFERENCE_MODEL_SEA1D) .and. TRANSVERSE_ISOTROPY) &
        stop 'models 1D_Sohl, IASP91, AK135, 1066A, JP1D and SEA1D are currently isotropic'
...
! Ebru - MARS
 if ( REFERENCE_1D_MODEL == REFERENCE_MODEL_SOHL) then
    ...
    RMOHO = 3280000.d0   ! 110 km average crustal thickness 
    R80  = 3055500.d0   ! 334.5 km deep, too small will cause negative Jacobian err
    R120 = -1.d0        ! by default there is no d120 discontinuity, except in
                        ! IASP91, therefore set to fictitious value
    R220 = 2908000.d0   ! d = 482 km  
    R400 = 2655000.d0   ! d = 735 km
    R600 = 2455000.d0   ! d = 935 km
    R670 = 2360000.d0   ! d = 1030 km
    R771 = 2033000.d0   ! d = 1357 km, below which the second doubling implemented
    RTOPDDOUBLEPRIME = 1503000.d0  ! dep = 1887 km, too thin for mesh ?
    RCMB = 1468000.d0   ! d = 1922 km
    !RICB = 400000.d0    ! d = 2990 km good for both stability and efficiency  
    RICB = 515000.d0    ! d = 2990 km good for both stability and efficiency  

    RHO_OCEANS = 1020.0 / RHOAV   ! will not be used
    ! densities fluid outer core (from modSOHL)
    RHO_TOP_OC = 6936.40 / RHOAV
    RHO_BOTTOM_OC = 7268.20 / RHOAV
! Ebru_end
...
!Ebru - a quick fix for MARS but should be done and checked again during 3D
!implementation
