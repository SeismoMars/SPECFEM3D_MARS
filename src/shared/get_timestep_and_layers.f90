...
!=====================================================================
  ...
  if (NEX_MAX*multiplication_factor <= 80) then
    ...
    !if (REFERENCE_1D_MODEL == REFERENCE_MODEL_SOHL) then
      R_CENTRAL_CUBE = 100000.d0
    !  NER_TOPDDOUBLEPRIME_771  = 7
    !  NER_OUTER_CORE           = 8
    !end if


  ! element width =   0.9375000      degrees =    55.46875      km
  else if (NEX_MAX*multiplication_factor <= 96) then
    ! time step
    !! DK DK to handle a case that Zhinan Xie found to be unstable for NEX = 96 I reduce 
    !! the time step to 90% of its value here
    !DT                       = 0.252d0 * 0.90d0
    !DT                       = 0.1d0 * 0.90d0
    DT                       = 0.252d0 * 0.90d0

    ! === Youyi Ruan MARS 08/25/2016 
    ! The shortest period at this resoution is about 30 s compare with 
    ! NEX=256, so change the attenuation period accordingly. 
    ! attenuation period range
    MIN_ATTENUATION_PERIOD   = 30
    MAX_ATTENUATION_PERIOD   = 1500

    !=== Youyi Ruan - MARS 05/15/2016
    NER_CRUST                = 1  ! 110 km
  ...
  ! element width =   0.5625000      degrees =    33.28125      km
  ...
  ! element width =   0.3515625      degrees =    20.80078      km
  else if (NEX_MAX*multiplication_factor <= 256) then
    ! Youyi Ruan MARS 07/23/2016
    !DT                       = 0.09d0
    DT                       = 0.2d0

    ! === Youyi Ruan MARS 08/29/2016 
    ! The shortest period at this resoution is about 12 s compare with 
    ! NEX=320, so change the attenuation period accordingly. 
    ! attenuation period range
    ...
    !if (REFERENCE_1D_MODEL == REFERENCE_MODEL_SOHL) then
    R_CENTRAL_CUBE = 395000.d0 !100000.d0

  ! element width =   0.2812500      degrees =    16.7113      km
  else if (NEX_MAX*multiplication_factor <= 320) then
    ! Youyi Ruan MARS 07/21/2016
    DT                       = 0.16 !0.07d0
  ...
  ! element width =   0.1875000      degrees =    20.84905      km
    ...
    stop 'NEX480 not implemented for Mars yet'
    DT                       = 0.09d0
  ...
  ! element width =   0.1757812      degrees =    19.54598      km
    ...
    stop 'NEX512 not implemented for Mars yet'
    DT                       = 0.1125d0
  ...
  ! element width =   0.1406250      degrees =    15.63679      km
    ...
    stop 'NEX640 not implemented for Mars yet'
    DT                       = 0.09d0
  ...
  ! element width =   0.1041667      degrees =    11.58280      km
    ...
    stop 'NEX864 not implemented for Mars yet'
    DT                       = 0.0667d0
  ...
  ! element width =   7.8125000E-02  degrees =    8.687103      km
    ...
    stop 'NEX1152 not implemented for Mars yet'
    DT                       = 0.05d0
  ...
  ! element width =   7.2115384E-02  degrees =    8.018865      km
    ...
    stop 'NEX1248 not implemented for Mars yet'
    DT                       = 0.0462d0
    ...
    stop 'NEX > 1248 not implemented for Mars yet'
  ...
  if (HONOR_1D_SPHERICAL_MOHO) then
    ...
    if (.not. ONE_CRUST) then
      ...
      ! Youyi Ruan MARS 07/27/16 overwrite previous setting
    ...
    ! 3D models: must have five element layers for crust
    !if (NER_CRUST < 2 ) NER_CRUST = 2
    !if (NER_CRUST < 3 ) NER_CRUST = 5

    if(ITYPE_CRUSTAL_MODEL == ICRUST_CRUSTMAPS .and. NER_CRUST < 3 ) NER_CRUST = 5
    ! Youyi Ruan MARS 12/05/16 overwrite previous setting 
...
!---
  ...
  if (CASE_3D .and. ITYPE_CRUSTAL_MODEL == ICRUST_CRUSTMAPS) &
    DT = DT*(1.d0 - 0.25d0)
  ...
  if (REGIONAL_MOHO_MESH) then
    ...
    ! checks
    ! === Youyi Ruan 11/05/2016 test for Mars globle using regional mesher
    !if (NCHUNKS > 1 ) stop 'regional moho mesh: NCHUNKS error in rcp_set_timestep_and_layers'
    if (HONOR_1D_SPHERICAL_MOHO ) return

    ! enforce 5 element layers
    ! === Youyi Ruan 11/14/2016 test for Mars globle using regional mesher
    NER_CRUST = 5
...
! multiply that time step by this ratio when LDDRK is on and when flag INCREASE_CFL_FOR_LDDRK is true.
  ...
  !=== Youyi Ruan 11/20/2016 MARS harded code DT for simple test
  !DT=0.0625
