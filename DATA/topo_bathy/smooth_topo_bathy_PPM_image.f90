...
! in practice though this simple filter works perfectly fine
  ...
  implicit none
!---  ETOPO4 30-minute model created by subsampling and smoothing etopo-2
! size of topography and bathymetry file
! integer, parameter :: NX_BATHY = 720,NY_BATHY = 360

!--- ETOPO10 10-minute model, implemented now, but data file must be created first
! size of topography and bathymetry file
...
! read the topography file
  ...
  open(unit=13,file='topo.10d540.Mars.dat',status='old')
  do iy=1,NY_BATHY
    do ix=1,NX_BATHY
...
! save the smoothed model
  if(SMOOTH_THE_MODEL) then
    ...
    open(unit=13,file='topo.10d540.Mars.dat.smoothed_window_3.dat',status='unknown')
    do iy=1,NY_BATHY
      do ix=1,NX_BATHY
